...
#ifndef SRC_DEPRECURSIVENN_H_
#define SRC_DEPRECURSIVENN_H_
#include "tensor.h"
...
/*
 * Full recursive in the projective dependency tree (PDT).
 */
template<typename xpu>
class DepRecursiveNN {
public:
...
public:
  ...
  /*
   * The difference from SemiDepRecursiveNN is "order". We firstly get the depths of each node,
   * then order these node by their depths from big to small, so "order" consists of the index
   * (start from 0) of each node and these indexes arrange according to the order mentioned above.
   * So "ComputeForwardScore" can be simply performed from "order" beginning to its end (the progress
   * is like from bottom to top in the PDT), and "ComputeBackwardLoss" can be performed from "order"
   * end to its beginning (the progress is like from top to bottom in the PDT).
   */
  inline void ComputeForwardScore(Tensor<xpu, 3, dtype> x, const vector<int> &dep,
		  const vector<string> &depType, const vector<int> order,
    ...
    if(seq_size != dep.size() || seq_size != depType.size() || seq_size != order.size() ||
    		seq_size != y.size(0) || seq_size != rsp.size(0)) {
        std::cerr << "DepRecursiveNN error: seq_size not equal" << std::endl;
        return;
    }

    if(x.size(3)!=_inputsize || rsp.size(3)!=_outputsize_rs) {
        std::cerr << "DepRecursiveNN error: size(3) not equal" << std::endl;
        return;
    }


    for(int idxOrder=0;idxOrder<seq_size;idxOrder++) {
    	int i = order[idxOrder];
		...
		for(int j=0;j<childrenIndex.size();j++) {
			...
			if (it != _map_r.end()) {
				_recursive_r[it->second].ComputeForwardScore(y[childrenIndex[j]], rsc[j]);
			} else {
				_recursive_r_other.ComputeForwardScore(y[childrenIndex[j]], rsc[j]);
			}
  ...
  inline void ComputeBackwardLoss(Tensor<xpu, 3, dtype> x, const vector<int> &dep,
    ...
    if(x.size(3)!=_inputsize || rsp.size(3)!=_outputsize_rs || lx.size(3)!=_inputsize ) {
        std::cerr << "DepRecursiveNN error: size(3) not equal" << std::endl;
        return;
    }
    ...
    for(int idxOrder=seq_size-1;idxOrder>=0;idxOrder--) {
    	int i = order[idxOrder];
    	vector<int> childrenIndex;
      	vector<string> childrenDepType;
    	for(int j=0;j<seq_size;j++) {
        ...
        for(int j=0;j<childrenIndex.size();j++) {
			...
			map<string, int>::iterator it = _map_r.find(childrenDepType[j]);
			if (it != _map_r.end()) {
				_recursive_r[it->second].ComputeBackwardLoss(y[childrenIndex[j]], rsc[j], lrsc, ly[childrenIndex[j]]);
			} else {
				_recursive_r_other.ComputeBackwardLoss(y[childrenIndex[j]], rsc[j], lrsc, ly[childrenIndex[j]]);
			}
