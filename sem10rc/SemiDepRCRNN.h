...
#ifndef SRC_SEMIDEPRCRNN_H_
#define SRC_SEMIDEPRCRNN_H_
#include "tensor.h"
...
/*
 * This is a semi-recursive Recurrent NN on the projective dependency tree (PDT).
 * Please reference 2015 EMNLP paper "Discriminative Neural Sentence Modeling by Tree-Based Convolution".
 * Give a sentence with its PDT, the main steps are as below:
 * 1. Get each node (word) representation by a recursive unit computing on the node and its children.
 * 2. Do recurrent computation on each node (word) representation by the order that it emerges in the
 * sentence.
 * We use "rs" for recursive and "rr" for recurrent, for short.
 */
...
public:
  ...
  /*
   * x denotes the word embedding sequence of the given sentence.
   * dep denotes the parent index of each word in the PDT. Please note that the original indexes
   * generated from Stanford denpendcy start from 1 (virtual node ROOT is 0). So you need to
   * decrease them by 1 when preprocessing.
   * There are two special situatoins. 1: some words may not emerge in the PDT, so their parent indexes
   * are -2 (after preprocessing). 2: The ROOT index is -1 (after preprocessing) and it does
   * not emerge in x.
   * depType denotes the dependency type between the word and its parent.
   * rsy denotes the word representation sequence after the recursive compuatations.
   * v_rsc should be empty and it will be filled by this function, but released by caller.
   * rsp should be Shape3(seq_size, 1, _outputsize_rs).
   * y denotes the output sequence after the recurrent computations.
   */
  inline void ComputeForwardScore(Tensor<xpu, 3, dtype> x, const vector<int> &dep,
		  ...
		  vector<Tensor<xpu, 3, dtype> > &v_rsc, Tensor<xpu, 3, dtype> rsp, Tensor<xpu, 3, dtype> y) {
    rsy = 0.0;
    rsp = 0.0;
	y = 0.0;
    ...
    // do recurrent
    _recurrent.ComputeForwardScore(rsy, y);
  ...
  inline void ComputeBackwardLoss(Tensor<xpu, 3, dtype> x, const vector<int> &dep,
    ...
    _recurrent.ComputeBackwardLoss(rsy, y, ly, lrsy);
