...
public:
  ...
  // the context size to represent embedding, default 1 means previous and next word to represent the current
  int wordcontext;
  int wordEmbSize;
  int otherEmbSize;
  int entity_embsize;
  ...
  int channelMode; // 000001-word, 000010-wordnet, 000100-brown, 001000-ner, 010000-pos, 100000-sst

  bool useDiscrete;

  int removePool; // to remove some pooling function, see Classifier

  int lossFunction; // 0-softmax, 1-rank
...
/*  virtual ~Options() {
  ...
  void setOptions(const vector<string> &vecOption) {
    ...
    for (; i < vecOption.size(); ++i) {
      ...
      else if(pr.first == "lossFunction")
    	  lossFunction = atoi(pr.second.c_str());
      else if(pr.first == "omitOther")
    	  omitOther = (pr.second == "true") ? true:false;
      else if(pr.first == "gamma")
    	  gamma = atof(pr.second.c_str());
      else if(pr.first == "mPos")
    	  mPos = atof(pr.second.c_str());
      else if(pr.first == "mNeg")
    	  mNeg = atof(pr.second.c_str());
  ...
  void showOptions() {
	...
	cout<<"lossFunction = "<<lossFunction<<endl;
	cout<<"omitOther = "<<omitOther<<endl;
