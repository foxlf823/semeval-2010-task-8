...
#ifndef SRC_PoolExGRNNClassifier_5TUPLE_attention1_H_
#define SRC_PoolExGRNNClassifier_5TUPLE_attention1_H_

#include <iostream>

#include <assert.h>

#include "AttRecursiveGatedNN_attention1.h"
#include "Example.h"
#include "Feature.h"
#include "N3L.h"
...
class PoolExGRNNClassifier_5tuple_attention1 {
public:
	PoolExGRNNClassifier_5tuple_attention1() {
    _dropOut = 0.5;
  }
  ~PoolExGRNNClassifier_5tuple_attention1() {
...
public:
  ...
  //overall
  AttRecursiveGatedNN_attention1<xpu> _target_attention;
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, int wordcontext, int rnnHiddenSize, int hiddensize, int labelSize) {
    ...
    cout<<"PoolExGRNNClassifier_5tuple_attention1 initial"<<endl;
  }
  ...
  inline dtype process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      Tensor<xpu, 2, dtype> former_reset_before, former_reset_middle, former_reset_after, former_interact_middle;
      Tensor<xpu, 2, dtype> former_update_before, former_update_middle, former_update_after, former_update_interact;
      Tensor<xpu, 2, dtype> former_interact, former_interactLoss;

      Tensor<xpu, 2, dtype> latter_reset_before, latter_reset_middle, latter_reset_after, latter_interact_middle;
      Tensor<xpu, 2, dtype> latter_update_before, latter_update_middle, latter_update_after, latter_update_interact;
      ...
      former_reset_before = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      former_reset_middle = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      former_reset_after = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      former_interact_middle = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      former_update_before = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      former_update_middle = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      former_update_after = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      ...
      latter_reset_before = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      latter_reset_middle = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      latter_reset_after = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      latter_interact_middle = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      latter_update_before = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      latter_update_middle = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      latter_update_after = NewTensor<xpu>(Shape2(1, _targetdim), 0.0);
      ...
      _target_attention.ComputeForwardScore(input_span[0], input_span[2], input_span[4],
    		  input_span[1],
            former_reset_before, former_reset_middle, former_reset_after, former_interact_middle,
			former_update_before, former_update_middle, former_update_after, former_update_interact,
			former_interact);

      _target_attention.ComputeForwardScore(input_span[0], input_span[2], input_span[4],
    		  input_span[3],
            latter_reset_before, latter_reset_middle, latter_reset_after, latter_interact_middle,
			latter_update_before, latter_update_middle, latter_update_after, latter_update_interact,
			latter_interact);
      ...
      FreeSpace(&former_reset_before);
      FreeSpace(&former_reset_middle);
      FreeSpace(&former_reset_after);
      FreeSpace(&former_interact_middle);
      FreeSpace(&former_update_before);
      FreeSpace(&former_update_middle);
      FreeSpace(&former_update_after);
      FreeSpace(&former_update_interact);
      FreeSpace(&(former_interact));
      FreeSpace(&(former_interactLoss));

      FreeSpace(&latter_reset_before);
      FreeSpace(&latter_reset_middle);
      FreeSpace(&latter_reset_after);
      FreeSpace(&latter_interact_middle);
      FreeSpace(&latter_update_before);
      FreeSpace(&latter_update_middle);
      FreeSpace(&latter_update_after);
