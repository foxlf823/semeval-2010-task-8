...
#ifndef SRC_CLASSIFIER_1_H_
#define SRC_CLASSIFIER_1_H_
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, Options options) {
    ...
    cout<<"do recurrent in the dependency path of the two entities"<<endl;
  }
  ...
  inline dtype process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      Tensor<xpu, 2, dtype> represent, representLoss;
      ...
      represent = NewTensor<xpu>(Shape2(1, _poolfunctions * _hiddensize), 0.0);
      representLoss = NewTensor<xpu>(Shape2(1, _poolfunctions * _hiddensize), 0.0);
