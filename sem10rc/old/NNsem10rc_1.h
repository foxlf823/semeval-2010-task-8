...
#ifndef NNSEM10RC_1_H_
#define NNSEM10RC_1_H_
...
#include "Classifier_3.h"
#include "Classifier_4.h"
#include "Classifier_5.h"
#include "Classifier_6.h"

using namespace nr;
using namespace std;
using namespace fox;
// do recurrent in the dependency path of the two entities
class NNsem10rc_1 {
public:
public:
	Options m_options;
...
#else
  //Classifier_1<cpu> m_classifier;
  //Classifier_2<cpu> m_classifier;
  //Classifier_3<cpu> m_classifier;
  //Classifier_4<cpu> m_classifier;
  //Classifier_5<cpu> m_classifier;
  Classifier_6<cpu> m_classifier;
#endif


  NNsem10rc_1(const Options &options):m_options(options)/*, m_classifier(options)*/ {
		nullkey = "-#null#-";
		unknownkey = "-#unknown#-";
		sentencekey = "-#sent#-";
	}
...
/*			m_classifier._ner.initial(nerEmb);
	...
	void initialExamples(Tool& tool, vector<Example>& examples) {
		...
		for(int egIdx=0;egIdx<examples.size();egIdx++) {
			...
			if(common==-2) {
				cout<<"warning: common -2, "<<examples[egIdx].text<<endl;
				continue;
			}

			// from former to ancestor
...
/*				Feature feature;
				featureName2ID(m_wordAlphabet, feature_word(word), feature.words);
				examples[egIdx].m_features.push_back(feature);*/
				featureName2ID(m_wordAlphabet, feature_word(word), examples[egIdx].m_before);
			}
			// from ancestor to latter
			//for(int sdpBNodeIdx=sdpB.size()-2;sdpBNodeIdx>=0;sdpBNodeIdx--) {
...
/*			for(int tkIdx=0;tkIdx<examples[egIdx].sent.tokens.size();tkIdx++) {
			...
			if(eg.formerTkBegin==-1 || eg.formerTkEnd==-1) {
				cout<<"warning: former -1, "<<examples[egIdx].text<<endl;
			}
			if(eg.latterTkBegin==-1 || eg.latterTkEnd==-1) {
				cout<<"warning: latter -1, "<<examples[egIdx].text<<endl;
			}*/
		}
