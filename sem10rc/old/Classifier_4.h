...
#ifndef SRC_CLASSIFIER_4_H_
#define SRC_CLASSIFIER_4_H_
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, Options options) {
    ...
    cout<<"do recurrent in from e1 to ancestor and e2 to ancestor"<<endl;
  }
  ...
  inline dtype process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      _rnn_left.ComputeForwardScore(inputLeft, rnn_hidden_left);
      _rnn_right.ComputeForwardScore(inputRight, rnn_hidden_right);
     ...
     // cout<<"pool "<<poolmerge.size(1)<<" "<<poolLeft[offset].size(1)<<endl;
      ...
      _rnn_left.ComputeBackwardLoss(inputLeft, rnn_hidden_left, rnn_hidden_leftLoss, inputLeftLoss);
      _rnn_right.ComputeBackwardLoss(inputRight, rnn_hidden_right, rnn_hidden_rightLoss, inputRightLoss);
